...
// Admin layout already redirects non-master_admin users to /admin/login,
// so the only auth concern here is attaching the admin JWT to every API call.
...
export default function AdminRepertoryUploadPage() {
  ...
  async function loadBooks() {
    ...
    try {
      const res = await fetch(`${API_URL}/api/repertory-upload/books`, { headers: authHeaders() })
  ...
  async function loadHistory() {
    ...
    try {
      const res = await fetch(`${API_URL}/api/repertory-upload/jobs/by-book?limit=200`, { headers: authHeaders() })
      const data = await res.json()
  ...
  async function loadParsers() {
    ...
    try {
      const res = await fetch(`${API_URL}/api/repertory-upload/parsers`, { headers: authHeaders() })
      const data = await res.json()
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/repertory-upload/required?parser=${encodeURIComponent(targetParserId)}`, { headers: authHeaders() })
  ...
  const allowedExtensions = useMemo(() => {
    ...
    if (exts.size === 0) { exts.add('.tab'); exts.add('.csv'); exts.add('.txt'); exts.add('.rtf'); exts.add('.doc') }
    return exts
  ...
  function streamProgress(id: number, startedAt: number) {
    ...
    // EventSource cannot send Authorization — pass the JWT via ?token= and let
    // the backend's requireAdmin middleware accept it from the query.
    const token = encodeURIComponent(getAdminToken())
    const ev = new EventSource(`${API_URL}/api/repertory-upload/jobs/${id}/stream?token=${token}`)
    sseRef.current = ev
    ev.onmessage = (msg) => {
      try {
